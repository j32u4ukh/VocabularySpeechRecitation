...
namespace VTS
{
    public class Utils
    {
        ...
        /// <returns></returns>
        static string debugMessage(string message, int line_num, string member, string file_path, bool use_timestamp = false)
        {
            ...
            if (use_timestamp)
            {
                ...
                return $"{timestamp} [{script_name}] {member} ({line_num}) | {message}";
            }
            else
            {
                return $"[{script_name}] {member} ({line_num}) | {message}";
            }
        }

        public static void log(string message = "", bool use_timestamp = false, [CallerLineNumber] int line_num = 0, [CallerMemberName] string member = "", [CallerFilePath] string file_path = "")
        {
            Debug.Log(debugMessage(message, line_num, member, file_path, use_timestamp));
        }

        public static void warn(string message, bool use_timestamp = false, [CallerLineNumber] int line_num = 0, [CallerMemberName] string member = "", [CallerFilePath] string file_path = "")
        {
            Debug.LogWarning(debugMessage(message, line_num, member, file_path, use_timestamp));
        }

        public static void error(string message, bool use_timestamp = false, [CallerLineNumber] int line_num = 0, [CallerMemberName] string member = "", [CallerFilePath] string file_path = "")
        {
            Debug.LogError(debugMessage(message, line_num, member, file_path, use_timestamp));
        }
        #endregion
    }
}
